...
// console.log(allSignUpFormFields);
...
function setErrorFor(input, message){
  ...
  // Remove Class Form Sucess From The Parent Target
...
newsignup=document.getElementById("signUpSubmitBtn")
console.log(newsignup)
newsignup.addEventListener("click",() =>{
if(localStorage.getItem("signUpSubmitBtn")==null){
...
signupArray.push(
  {
    ...
    'passward':document.getElementById("signUpPassword").value,
    'confirmPassword':document.getElementById("signUpConfirmPassword").value
  }
);
localStorage.setItem("singup",JSON.stringify(signupArray));
   }else{
  signupArra=localStorage.getItem("signup");
  signupArr=JSON.parse(signupArra);
  signupArr.push(
    {
      'username':string(ngdocument.getElementById("signUpUsername").value),
       'email':document.getElementById("signUpEmail").value,
       'passward':string(document.getElementById("signUpPassword").value),
       'confirmPassword':string(document.getElementById("signUpConfirmPassword").value)
    }
  );
...
// login data store
validate=false;
let elementUName, elementPPassward
function login(){
  debugger;
  let emaill=document.getElementById("loginUsername").value;
  let passwardd=document.getElementById("loginPassword").value;
     signUpstr=localStorage.getItem("singup");
     signupArr=JSON.parse(signUpstr)
     
    {
         if (emaill ==signupArr[0].username  && passwardd == signupArr[0].passward) {
           validate = true;
